...
class Cart extends Common
{
        ...
        if($this->checkToken()){
            ...
            if($uid){
                ...
                if($goods['store']>=$goodsNum){
                    //判断商品是否存在购物车当中，如果存在则添加数量，不存在则添加数据
    ...
    public function orderInfo(){
        if($this->checkToken()){
            $cartIds = input('cartIds','');//传入购物车的id
            $openid = input('openid','');
            $uid = getUidByOpenid($openid);
            //购物车,in查询，包含$cartIds的全部
    ...
    public function submitOrder(){
        if($this->checkToken()){
            ...
            //添加订单和订单商品
            $result = $this->addOrder($uid, $addressId, $amount, $cartIds);
    ...
    public function getWxpayInfo($order_sn_submit){
        /**
         * 第一步：先进行预支付：需要获取订单名称(商品名称)、订单号、金额、openid
         * 第二步：对支付的结果再次签名调起支付：需要prepay_id
         */
        //根据order_sn_submit(订单号)获取订单信息
    ...
    public function prePay($body, $out_trade_no, $total_fee, $open_id) {
        /**
         * $body(商品名),$out_trade_no(订单号),
         * $total_fee(订单金额),$open_id(openid)
         */
        //微信支付提供的支付接口
        $url = 'https://api.mch.weixin.qq.com/pay/unifiedorder';
        $notify_url = config('wxpay.notify_url');
        $nonce_str = $this->getRandChar(32);//随机数
